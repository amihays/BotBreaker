(function () {
  ...
  FixedBoundary.prototype.restorePosition = function (vertex) {
    ...
    var projectedVec = normVec.scale(projectedVel * -1.0 * (1 + this.bounceCoeff));
    var correction = .1;

    vertex.vel = vertex.vel.add(projectedVec);
    vertex.vel = vertex.vel.add(normVec.scale(1.0 * correction))
    // this.drawMoreShit(vertex.pos, projectedVec, 1000);
    // this.drawMoreShit(vertex.pos, vertex.vel, 1);
  }
