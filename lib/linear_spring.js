(function () {
  ...
  var LinearSpring = BB.LinearSpring = function (vertex1, vertex2, stiffness, damping, equilLength) {
    ...
    this.vertex1.spring = this;
    this.vertex2 = vertex2;
    this.vertex2.spring = this;
    this.equilLength = equilLength || this.calcEquilLength();
    BB.Spring.call(this, stiffness, damping);
  }

  LinearSpring.prototype.calcEquilLength = function () {
    return this.vertex1.pos.subtract(this.vertex2.pos).magnitude();
  }
  ...
  LinearSpring.prototype.dampVelocities = function () {
    var direction = this.springVector().unitVector();
    //
    ...
    var damp_vel = direction.scale(0.5 * vel_dot_dir);
    // console.log(damp_vel);
    this.vertex1.vel = this.vertex1.vel.add(damp_vel.scale(this.damping));
    this.vertex2.vel = this.vertex2.vel.add(damp_vel.scale(-this.damping));
  }
