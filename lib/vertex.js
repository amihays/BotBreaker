(function () {
  ...
  Vertex.prototype.move = function (dt) {
    ...
    if (this.boundary) {
      ...
      var normVec = boundaryDir.getNormalDirection();

      var projectedVel = normVec.dot(this.vel);
      var projectedVec = normVec.scale(projectedVel * -1.0 * (1));

      var projectedForce = normVec.dot(this.force);
      var projectedForceVec = normVec.scale(projectedForce * -1.0);
