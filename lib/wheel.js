(function () {
  ...
  Wheel.prototype.buildAxelFrameSprings = function () {
    var innerAxelFrameStiffness = 20;
    var damping = 0.99;

    Object.keys(this.axelFrameVertices).forEach(function (location) {
      var vertex = this.axelFrameVertices[location];
      var spring = new BB.LinearSpring(vertex, this.axelVertex, innerAxelFrameStiffness, damping);
      this.springs.push(spring);
    }.bind(this))

    var outerAxelFrameStiffness = 20;
    ...
    var topToRightSpring = new BB.LinearSpring(top, right, outerAxelFrameStiffness, damping);
    var rightToBottomSpring = new BB.LinearSpring(right, bottom, outerAxelFrameStiffness, damping);
    var bottomToLeftSpring = new BB.LinearSpring(bottom, left, outerAxelFrameStiffness, damping);
    var leftToTopSpring = new BB.LinearSpring(left, top, outerAxelFrameStiffness, damping);
  ...
  Wheel.prototype.buildTire = function (numSpokes, radius, scale) {
    ...
    for(var n = 0; n < numSpokes; n++) {
      ...
      if (n > 0) {
        var outerSpring = new BB.LinearSpring(vertices[n], vertices[n - 1], tireStiffness, damping);
        this.springs.push(outerSpring);
      }
      var innerSpring = new BB.LinearSpring(vertices[n], this.axelVertex, tireStiffness, damping);
      this.springs.push(innerSpring);
    }
    var lastOuterSpring = new BB.LinearSpring(vertices[numSpokes - 1], vertices[0], tireStiffness, damping);
